...
reconvergences (a diamond yields exact $5/6$ versus soft-OR $7/10$); the proof is formalised in
Lean~4. Second, the Gene Ontology
...
exact marginal is $5/6$ while the soft-OR gives $7/10$. The development is formalised in
...
The formalisation comprises a concrete Lean-core development and a parametric \texttt{mathlib} development over all $k$ and all real priors; its split, axiom footprint, and lemma inventory are in Appendix~\ref{app:lean}.
...
The construction generalizes the cc calibration---an exact SDD per module, weighted marginals over the low-treewidth separators, the bounded soft closure (Theorem~\ref{thm:star}) only across the residual ${\le}8\%$ core. We run it rather than infer it: on mf, isolating the $2.95\%$ core leaves $488$ modules (max treewidth $11$) over which exact junction-tree WMC computes all $6{,}645$ marginals---$97\%$ of mf---in $1.1$\,s, validated to $10^{-16}$ against brute force; on bp---treewidth $272$, where no monolithic circuit can be built---the $7.7\%$ cut leaves $2{,}210$ modules (max treewidth $8$) over which exact WMC computes the remaining $19{,}705$ marginals ($92\%$ of bp) in $12.8$\,s, again machine-precision exact. The net effect is exact WMC over $92$--$100\%$ of GO with a single small locus of bounded approximation.

\paragraph{Two costs, one separator: a rank-$r$ dial for the residual core.}
Scaling exact WMC fights two costs. The \emph{circuit size} ($2^{\tw}$) the modular construction defeats wherever separators are small. The \emph{dependence} the layer may represent is the second: at the residual core the soft closure merges the incoming separator messages as a rank-$1$ product---the independence assumption \emph{at the boundary}, precisely why BP returns $7/10$ where exact WMC is $5/6$ (Theorem~\ref{thm:star}). The error is not intrinsic to the core but to representing its (non-product, \emph{low-rank}) boundary joint as independent; a rank-$r$ separator message---a tensor train over the separator atoms---dials the residual to zero ($r{=}1$ soft closure, full rank exact).

We measure this on a real GO cellular-component reconvergence core ($13$ terms, $6$ multi-parent, from \texttt{go.obo}; the diamond is its rank-$2$ instance, reproducing the Lean $5/6$). Its exact constraint-induced boundary joint ($280$ models) is strongly non-product---the rank-$1$ soft-closure representation discards $1.38$ nats (total variation $0.62$)---yet has exact tensor-train rank $10$ (vs $2^{13}{=}8192$ dense), so a rank-$r$ boundary head dials the residual to exact at rank $10$ (full dial in Table~\ref{tab:rank}, App.~\ref{app:nesy}), the per-module SDDs unchanged. Across $40$ such cc cores the effect is systematic: IA gap median $1.41$ nats ($0.93$--$2.36$), exact rank median $11$ (max $27$), always far below the $2^{13}$ dense size. The same separator interface that buys tractability (Proposition~\ref{prop:modular}) thus also bounds the cost of \emph{exact} dependence. It is modest here because the monotone true-path theory yields a \emph{unimodal}, merely correlated joint; the qualitative break is disjunction. A SROIQ ontology's covering and disjointness axioms make the boundary joint \emph{multimodal}---which the IA provably cannot represent and where the soft closure ceases to be BP; in a controlled disjunctive fragment the IA discards $2.08$ nats and is exact only at full rank (App.~\ref{app:nesy}). Yet the separator merely carries \emph{disjunctive contexts} in place of Boolean consequences, leaving the modular interface and its rank-$r$ dial unchanged. EL is the exact base case; SROIQ is the Cost-II frontier the same framework reaches (\S\ref{sec:nesy}).
...
ask where the canonical NeSy frameworks sit. The compilation- and provenance-based engines run on the
\emph{identical} GO true-path inference: we encode $T$ natively in DeepProbLog and Scallop and
validate against our exact marginals (App.~\ref{app:nesy}), so any divergence is the inference
strategy, not the encoding. The learned approximators (A-NeSI, NeSyDM) cannot be run on GO at
all---they need a trained model per ontology and, for A-NeSI, samples of the exact $\WMC$ we are
computing---so we position them by their documented collapse on coupled structure. Each trades
correctness for scale in its own way (Table~\ref{tab:nesy}).

\begin{table}[t]\centering
\caption{Where NeSy frameworks sit on ontology-scale WMC. DeepProbLog and Scallop run on the GO true-path inference; A-NeSI/NeSyDM cannot (they need training, and A-NeSI needs exact-$\WMC$ samples), and are placed by their coupled-structure collapse.}\label{tab:nesy}
\small
\begin{tabular}{lcc}
\toprule
...
thus keeps a single column of Table~\ref{tab:nesy}; ours keeps both---exact on the
...
For the true-path theory the soft closure is belief propagation; we make it a deployable layer---exact on the tree fragment, modular-exact over $92$--$100\%$ of GO with the residual core rank-$r$-dialable to exact---uniquely both scalable and accuracy-characterized.
...
\paragraph{Two developments and a decidable rational arithmetic.}
...
$\mathrm{KL}(q^\star\Vert\text{product})$ is $1.41$ nats in the median (range $0.93$--$2.36$),
and the joint's exact rank is $11$ in the median (range $7$--$27$), always far below the $2^{13}$
...
disjointness---the exact joint is \emph{multimodal} ($5$ modes), and the IA cannot represent it:
the best product discards $2.08$ nats (total variation $0.86$, $3.8\%$ error on a composed query) and
...
\begin{table}[t]\centering
\caption{Cost-II in the disjunctive (SROIQ) regime: covering $+$ disjointness over $w{=}6$ atoms give a $5$-mode joint the IA cannot represent; a rank-$r$ separator message is exact at the joint's rank $3$.}\label{tab:disj}
\small
\begin{tabular}{lccc}
\toprule
boundary model & $\mathrm{KL}(q^\star\Vert\cdot)$ & $\mathrm{TV}$ & WMC rel.\ err.\\
\midrule
IA (best product) & $2.08$ & $0.86$ & $3.8\times10^{-2}$\\
rank-$2$ & $0.08$ & $0.09$ & $3.7\times10^{-2}$\\
