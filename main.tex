...
$0.96\!\to\!0.03$--$0.13$ from $N{=}1$ to $N{=}4$) -- ours is alone in being both scalable and
...
therefore keeps a single column of Table~\ref{tab:nesy}; ours keeps both -- exact on the
...
The four-node reconvergent derivation has two proof paths to a shared fact via the upward rule $\texttt{holds}(P)\!:\!-\,\texttt{sub}(C,P),\texttt{holds}(C)$ at edge-probability $\tfrac12$ -- a construction distinct from the Lean true-path diamond of \S\ref{sec:lean} (different inference semantics, so its exact value $0.9375$ does not conflict with that diamond's $5/6$). The exact marginal $P(\texttt{holds}(a))=0.9375$ is approximated as $0.50$ at $k{=}1$ (a $47\%$ undercount), recovers to $0.875$ at $k{=}3$, and returns to the exact $0.9375$ at $k{=}10$. On $14$-node biological-process neighborhoods the maximum drift versus the exact reference falls from $0.15$--$0.30$ at $k{=}1$ to $0.000$ at $k{=}10$. The error therefore grows with proof multiplicity (reconvergence) and shrinks as $k$ admits more proofs -- the same reconvergence axis along which our soft closure departs from exact $\WMC$.
